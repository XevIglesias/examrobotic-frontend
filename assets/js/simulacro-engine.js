...
function startExam() {
    ...
    let pdfPool = [...SIMULACRO_PDF_DATA].sort(() => Math.random() - 0.5).slice(0, 32);
    let existPool = [...EXAM_DATA].sort(() => Math.random() - 0.5).slice(0, 8);
    activeQs = [...pdfPool, ...existPool].sort(() => Math.random() - 0.5);
...
function finish() {
    ...
    if (typeof saveAttempt === 'function') saveAttempt(slug, 'simulacro', parseFloat(final), score, timeSpent, userAns);
