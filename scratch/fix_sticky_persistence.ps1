# Script to fix sticky behavior (remove items-start) and unify headers
...
"@
    ...
    # 2. Fix Sticky Behavior (remove items-start to allow aside to stretch)
    # We replace 'flex-col xl:flex-row gap-8 items-start' with 'flex-col xl:flex-row gap-8'
    $content = $content.Replace("flex flex-col xl:flex-row gap-8 items-start", "flex flex-col xl:flex-row gap-8")
    
    # 3. Ensure Aside container is transparent or doesn't have height constraints
    # (The sticky div inside will do the work)
